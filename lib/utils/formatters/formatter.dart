...
class TFormatter {
  ...
  static String formatSleepTime(PickedTime time, bool formatTo24Hours) {
    if (formatTo24Hours) {
      return '${intl.NumberFormat('0').format(time.h)}h : ${intl.NumberFormat('00').format(time.m)}m';
    } else {
      return '${intl.NumberFormat('0').format(time.h % 12 == 0 ? 12 : time.h % 12)}h : ${intl.NumberFormat('00').format(time.m)}m';
    }
  }
  ...
  // Convert String to Pickedtime
  static PickedTime formatStringSleepTime(String sleepTime) {
    try {
      // Split the string using a regular expression to capture numbers
      RegExp regex = RegExp(r'(\d+)');
      List<String> parts =
          regex.allMatches(sleepTime).map((m) => m.group(0)!).toList();

      if (parts.length != 2) {
        TLoggerHelper.error('Invalid time format');
      }
